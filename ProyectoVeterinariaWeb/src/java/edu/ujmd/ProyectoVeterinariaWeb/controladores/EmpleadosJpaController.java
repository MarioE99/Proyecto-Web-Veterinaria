...
import edu.ujmd.ProyectoVeterinariaWeb.entidades.Citas;
...
public class EmpleadosJpaController implements Serializable {
    ...
    public void create(Empleados empleados) {
        ...
        try {
            ...
            CargoEmpleado codCargo = empleados.getCodCargo();
            if (codCargo != null) {
                codCargo = em.getReference(codCargo.getClass(), codCargo.getCodCargo());
                empleados.setCodCargo(codCargo);
            }
            Turno codTurno = empleados.getCodTurno();
            if (codTurno != null) {
                codTurno = em.getReference(codTurno.getClass(), codTurno.getCodTurno());
                empleados.setCodTurno(codTurno);
            }
            ...
            if (codCargo != null) {
                codCargo.getEmpleadosList().add(empleados);
                codCargo = em.merge(codCargo);
            }
            if (codTurno != null) {
                codTurno.getEmpleadosList().add(empleados);
                codTurno = em.merge(codTurno);
            }
    ...
    public void edit(Empleados empleados) throws IllegalOrphanException, NonexistentEntityException, Exception {
        ...
        try {
            ...
            CargoEmpleado codCargoOld = persistentEmpleados.getCodCargo();
            CargoEmpleado codCargoNew = empleados.getCodCargo();
            Turno codTurnoOld = persistentEmpleados.getCodTurno();
            Turno codTurnoNew = empleados.getCodTurno();
            ...
            if (codCargoNew != null) {
                ...
                empleados.setCodCargo(codCargoNew);
            }
            if (codTurnoNew != null) {
                codTurnoNew = em.getReference(codTurnoNew.getClass(), codTurnoNew.getCodTurno());
                empleados.setCodTurno(codTurnoNew);
            }
            ...
            if (codCargoOld != null && !codCargoOld.equals(codCargoNew)) {
                codCargoOld.getEmpleadosList().remove(empleados);
                codCargoOld = em.merge(codCargoOld);
            }
            if (codCargoNew != null && !codCargoNew.equals(codCargoOld)) {
                codCargoNew.getEmpleadosList().add(empleados);
                codCargoNew = em.merge(codCargoNew);
            }
            if (codTurnoOld != null && !codTurnoOld.equals(codTurnoNew)) {
                codTurnoOld.getEmpleadosList().remove(empleados);
                codTurnoOld = em.merge(codTurnoOld);
            }
            if (codTurnoNew != null && !codTurnoNew.equals(codTurnoOld)) {
                codTurnoNew.getEmpleadosList().add(empleados);
                codTurnoNew = em.merge(codTurnoNew);
            }
    ...
    public void destroy(Integer id) throws IllegalOrphanException, NonexistentEntityException {
        ...
        try {
            ...
            CargoEmpleado codCargo = empleados.getCodCargo();
            if (codCargo != null) {
                codCargo.getEmpleadosList().remove(empleados);
                codCargo = em.merge(codCargo);
            }
            Turno codTurno = empleados.getCodTurno();
            if (codTurno != null) {
                codTurno.getEmpleadosList().remove(empleados);
                codTurno = em.merge(codTurno);
            }
            em.remove(empleados);
            em.getTransaction().commit();
